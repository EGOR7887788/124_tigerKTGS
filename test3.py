# create
a = [1, 11, 'a', [1], (1, 11), {'a':1}, None, True] # список со всем
a = [] # пустой список
b = list() # пустой список
a = (1, 2.1, 3) # раньше я был кортежем
list(a) # [1, 2.1, 3], но 'a' осталась кортежем
b = list('abc') # ['a', 'b', 'c']
# retrieve (read)
a = [1, 1.1, 'a']
print(a) # [1, 1.1, 'a']
...
a = [1, 2, 3]
a.рор() # Возвращает последний элемент и удаляет его из списка. В примере вернёт 3 и удалит его из списка. Теперь "а" [1, 2]
a = [1, 'ab', 3]
a.рор(1) # Возвращает элемент по указанному ИНДЕКСУ и удаляет его из списка. В примере вернёт 'ab' и удалит его из списка. Теперь "а" [1, 3]. Если такого индекса нет, то возникнет ошибка IndexError: pop index out of range
a = [1, 2, 'ab']
